---
title: Google Workspace Login Activity Input
description: Collect Google Workspace Login Activity logs from your Google Workspace Login Activity API
---
...
6. **Create and Configure an Admin Account**:
    ...
    - Name the role (e.g., "Login Activity Report Reader").
...
The Google Workspace Login Activity Input is configured using the following settings:
...
curl -X 'POST' \
 ...
 "name": "input_name",
 "promise_id": "",
 "type": "google-workspace-login-activity"
}'
```
...
2. Periodically fetches login activity logs.
3. Processes and publishes login activity data to the pipeline for further handling.
...
- No data being fetched could indicate a lack of login activities or logging issues in your Google Workspace.
- 
