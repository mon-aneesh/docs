---
title: Duo Security Authentication Logs
description: >
 Returns a list of Authentication log events from Duo Security.
---

## Details
Monad uses the `mintime` on the [api](https://duo.com/docs/adminapi#logs) to determine what logs to display. This field is updated every time a request to get authentication logs is successful with the last time a request to get the logs was initiated. If this was the first time requesting for admin logs, a full sync of the data is performed.
