...
- Instructions [Here](https://login.tailscale.com/admin/settings/oauth).
  ...
  - In the Generate OAuth client page, Select `users:read` scope.
...
- You can view your [Organization name](https://tailscale.com/kb/1217/tailnet-name#organization-name) in the [Settings](https://login.tailscale.com/admin/settings) page of the admin console. Leave the field blank if you want users from default organization.


## Related Articles
- [https://tailscale.com/api#description/overview](https://tailscale.com/api#description/overview)
- [https://tailscale.com/api#tag/users/GET/tailnet/{tailnet}/users](https://tailscale.com/api#tag/users/GET/tailnet/{tailnet}/users)
...
| User Role                 | string | Yes      | Allows for filtering the output by user role.             |
| User Type                 | string | Yes      | Allows for filtering the output by user type             |
...
To send a `POST` request to create this Tailscale Users Connector.
...
curl -X 'POST' \
  ...
  "config": {
    ...
    "settings": {
      ...
      "user_role_option": "one of `owner` `member` `admin` `it-admin` `network-admin` `billing-admin` `auditor` or `all`",
      "user_type_option": "one of `member` `shared` or `all`"
