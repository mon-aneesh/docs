---
title: Google Workspace Drive Activity Input
description: Collect Google Workspace Drive Activity logs from your Google Workspace Drive Activity API
---

# Google Workspace Drive Activity Input

The Google Workspace Drive Activity Input allows you to collect drive activity logs from your Google Workspace environment. This input utilizes the Google Workspace Admin SDK Reports API to fetch drive activity data, providing insights into file and folder actions within your organization's Google Drive.
...
6. **Create and Configure an Admin Account**:
    ...
    - Name the role (e.g., "Drive Activity Report Reader").
...
The Google Workspace Drive Activity Input is configured using the following settings:
...
curl -X 'POST' \
 ...
 "name": "input_name",
 "promise_id": "",
 "type": "google-workspace-drive-activity"
}'
```
...
2. Periodically fetches drive activity logs.
3. Processes and publishes drive activity data to the pipeline for further handling.
...
- No data being fetched could indicate a lack of drive activities or logging issues in your Google Workspace.
