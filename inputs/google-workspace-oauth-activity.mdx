---
title: Google Workspace OAuth Activity Input
description: Collect Google Workspace OAuth Activity logs from your Google Workspace OAuth Activity API
---
...
6. **Create and Configure an Admin Account**:
    ...
    - Name the role (e.g., "OAuth Activity Report Reader").
...
The Google Workspace OAuth Activity Input is configured using the following settings:
...
curl -X 'POST' \
 ...
 "name": "input_name",
 "promise_id": "",
 "type": "google-workspace-oauth-activity"
}'
```
...
2. Periodically fetches OAuth activity logs.
3. Processes and publishes OAuth activity data to the pipeline for further handling.
...
- No data being fetched could indicate a lack of OAuth activities or logging issues in your Google Workspace.
